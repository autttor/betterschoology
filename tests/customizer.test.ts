...
describe('customizer controls', () => {
  it('uses the local display name and makes native header replacement optional', async () => {
    ...
    const headerToggle = checkbox('Apply display name to Schoology header');
  ...
  it('exposes dashboard sections without requiring all of them', async () => {
    ...
    expect(checkbox('Show Better To Do').checked).toBe(true);
    expect(checkbox('Show Notifications').checked).toBe(true);
    expect(checkbox('Show Recent Feedback').checked).toBe(true);
    expect(checkbox('Show Announcements').checked).toBe(false);
    await act(async () => { checkbox('Show Notifications').click(); });
